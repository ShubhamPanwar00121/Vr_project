...
namespace Assets.Project.ChessEngine
{
    public class Board
    {
        ...
        public bool IsSquareAttacked(Square sq, Color side)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                while (!(piece is OffLimits))
                {
                    if (piece != null)
                    {
                        if ((piece is Pieces.Rook || piece is Queen) && piece.Color == side)
                        {
                            return true;
                        }
                        break;
        ...
        {
            Bishop.GetIndex(Color.White), ChessEngine.Pieces.Rook.GetIndex(Color.White), Queen.GetIndex(Color.White), null,
            Bishop.GetIndex(Color.Black), ChessEngine.Pieces.Rook.GetIndex(Color.Black), Queen.GetIndex(Color.Black), null,
        };
        ...
        public bool DoMove(Move move)
        {
            ...
            HashSide();
            //if the kings square is attacked and the king cannot move then checkmate
        ...
        public int EvaluatePosition()
        {
            ...
            //rooks
            pieceIndex = ChessEngine.Pieces.Rook.GetIndex(Color.White);
            foreach (Piece piece in PieceList.GetList(pieceIndex))
            {
                Square square = piece.Square;
                score += RookPositionValue[Sq64((int)square)];
            }
            pieceIndex = ChessEngine.Pieces.Rook.GetIndex(Color.Black);
            foreach (Piece piece in PieceList.GetList(pieceIndex))
            {
                Square square = piece.Square;
        ...
        private int AlphaBeta(int alpha, int beta, int depth, SearchInfo info)
        {
            ...
            {
                //this is where its mate for player
